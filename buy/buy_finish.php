...
EOM;
    ...
    //購入完了のメールを購入者へ送信
    $mailTo = $mail_2; //メールの宛先
    $subject = "【iCycle】購入完了のお知らせ";  //題名
    //メールの内容
    $body = <<< EOM
    購入が完了しました。
EOM;
    ...
    //購入成立の知らせを運営に送信
    $mailTo = MAIL; //メールの宛先
    $subject = "購入成立のお知らせ";  //題名


    $body = <<< EOM
    ユーザー名（{$username}）さんの商品（{$pname}）を、ユーザー名（{$_SESSION['username']}）さんが購入。
EOM;
...
<head>
  ...
  <title>購入完了画面|学内フリマ「iCycle」</title>
