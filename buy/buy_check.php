...
if(!isset($_SESSION['username'])) {
  ...
  try{
    ...
    if($statement->rowCount() == 0) {
      $errors['buy'] = "この商品は既に購入または削除されています。";
    }
...
<head>
  ...
  <title>購入確認画面|学内フリマ「iCycle」</title>
    ...
    <div class="main-center">
      ...
      <?php if (count($errors) === 0): ?>
        ...
        <p>を購入しますか？<p>
          ...
          <div class="next">
            <form  class="back" action="../product.php" method="post">
              <input type="hidden" name="username" value="<?= h($username)?>">
              <input type="hidden" name="pname" value="<?= h($pname)?>">
              <input type="submit" value="戻る">
            </form>
            <form action="buy_finish.php" method="post">
              <!-- <input type="hidden" name="image0" value="<?=h($image0)?>">
              <input type="hidden" name="image1" value="<?=h($image1)?>">
              <input type="hidden" name="image2" value="<?=h($image2)?>">
              <input type="hidden" name="image3" value="<?=h($image3)?>"> -->
              <input type="hidden" name="username" value="<?=h($username)?>">
              <input type="hidden" name="pname" value="<?=h($pname)?>">
              <input type="hidden" name="token" value="<?=h($_POST['token'])?>">  <!--token送信忘れずに、h使うべき？-->
